...
export const createUserMdl = function (userData, callback) {
  ...
  execQuery(db, checkEmailQuery, function (err, results) {
    if (err) {
      ...
      const checkUsernameQuery = `SELECT COUNT(*) AS usernameCount FROM user WHERE userName = "${userName}"`;
      ...
      execQuery(db, checkUsernameQuery, function (err, results) {
        if (err) {
          ...
          if (emailCount > 0) {
            ...
            const insertUserQuery = `INSERT INTO user (userName, firstName, lastName, email, password, mobileNo, address) VALUES ("${userName}", "${firstName}", "${lastName}", "${email}", "${password}", "${mobileNo}", "${address}")`;
